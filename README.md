...
想知道一只精灵好不好抓，查两个字段：
- `Catch_Threshold`：阈值越高越容易抓。喵喵是 1000，恶魔狼只有 250
- `catch_guarant_rate`：保底率越高，多扔几个球越容易保底抓到。喵喵是 1700（17%），恶魔狼只有 180（1.8%）

最终捕获概率还要结合 `ball_conf.csv` 里球的 `ball_prob` 和 `ball_threshold_modify` 一起算。简单理解：**球参数 × 精灵阈值 → 最终捕获概率**。
